...
from required import require_htno
import openpyxl 
import math
...
def create_pdf(filename):
    ...
    wb = openpyxl.load_workbook("Input Format.xlsx")
    ...
    for k in range(2, sheet2_row + 1):
        if sheet2.cell(k, 2).value == "Regular":
            ...
            count = 0
            length = length_dataFrame(sheet2.cell(k, 1).value, sheet2.cell(k, 3).value)
            x = length / 36
            y = math.ceil(x)
            ...
            for page_num in range(y):  # Loop over pages
                ...
                text1="JAWAHARLAL NEHRU TECHNOLOGICAL UBIVERSITY - KAKINADA - 533 003"
                c.drawString(rect1_x+100,page_height-top_margin-height+5,text1)


                # Draw the second rectangle
                c.rect(rect2_x, rect2_y, width-150, height, fill=0)
                text2="Name of the Examination"
                c.drawString(rect2_x+250,page_height-top_margin-(2*height)+5,text2)
...
def length_dataFrame(branch_code,subject_code):
    data = require_htno()
    length = len(data['0'+str(branch_code)][subject_code])
    return length

create_pdf("seating_arrangement.pdf")